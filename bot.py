...
admin_id = admin_id
bot.set_my_commands(set_def_commands)
...
def question_user(message): 
    ...
    check = db_commands.check_id(user_id= message.from_user.id)
    if check:
        bot.send_message(message.chat.id, 'Вы являетесь администратором, а не пользователем!')
    else:    
        if user_question:
            matched_answers = []  
...
def show_questions(message):
    ...
    if check:
        ...
        for question in questions:
            ...
            markup.add(btn)
        bot.send_message(message.chat.id, "Выберите вопрос на который хотите ответить:", reply_markup=markup)
    else:
...
def handle_admin_reply(message):
    ...
    if question:
        user_id = question[1]
        but1 = InlineKeyboardButton("Вопрос не был решен", callback_data=f"actual_{question_id}")
...
def update_status(call):
    ...
    if call.data == f'resolved_{question_id}': 
        ...
        bot.send_message(call.message.chat.id,'Статус вашего вопроса остается "ожидающие ответа поддержки", ожидайте ответа поддержки')
...
def resolved_questions(message):
    ...
    if check:
        if not resolv_questions:
            bot.send_message(message.chat.id,'В данный момент все вопросы ожидают вашего ответа')
            return
        markup = InlineKeyboardMarkup()
...
def del_callback(call):
    ...
    if check:
        ...
        markup.add(but1)
        bot.send_message(call.message.chat.id, "Секунду удаляю вопрос:", reply_markup=markup)
    else:
        bot.send_message(call.message.chat.id,'У тебя нет доступа к этой функции!')
